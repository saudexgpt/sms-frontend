<template>
  ...
  <vs-dropdown vs-custom-content vs-trigger-click class="cursor-pointer">
    <feather-icon :badge="unreadNotifications.length" icon="BellIcon" class="cursor-pointer mt-1 sm:mr-6 mr-2" />
        ...
        <ul class="bordered-items">
          <li v-for="ntf in unreadNotifications" :key="ntf.index" class="flex justify-between px-4 py-4 notification cursor-pointer">
            <div class="flex items-start">
              <feather-icon :icon="ntf.icon" :svg-classes="[`text-${ntf.category}`, 'stroke-current mr-1 h-6 w-6']"/>
              <div class="mx-2">
                <span :class="[`text-${ntf.category}`]" class="font-medium block notification-title">{{ ntf.title }}</span>
                <small>{{ ntf.msg }}</small>
              </div>
            </div>
