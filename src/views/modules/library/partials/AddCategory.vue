<template>
  <div>
    <b-sidebar
      ...
      :visible="isCreateCategorySidebarActive"
      bg-variant="white"
      shadow
      backdrop
      no-header
      right
      @change="(val) => $emit('update:is-create-category-sidebar-active', val)"
    >
...
export default {
  ...
  model: {
    prop: 'isCreateCategorySidebarActive',
    event: 'update:is-create-category-sidebar-active',
  },
  props: {
  ...
  methods: {
    ...
    submit() {
      const app = this
      const saveCategoryResource = new Resource('library/books/store-category')
      const param = app.form
      if (param.name === '') {
        app.$alert('The Category Name field is mandatory')
      } else {
        app.loading = true
        saveCategoryResource.store(param)
          .then(response => {
            app.loading = false
            this.$message({
              type: 'success',
            ...
            app.$emit('update:is-create-category-sidebar-active', false)
          }).catch(error => {
            app.error = true
