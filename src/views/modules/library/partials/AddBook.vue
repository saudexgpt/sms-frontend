<template>
  <div>
    <b-sidebar
      ...
      :visible="isCreateBookSidebarActive"
      bg-variant="white"
      shadow
      backdrop
      no-header
      right
      @change="(val) => $emit('update:is-create-book-sidebar-active', val)"
    >
...
export default {
  ...
  model: {
    prop: 'isCreateBookSidebarActive',
    event: 'update:is-create-book-sidebar-active',
  },
  props: {
    isCreateBookSidebarActive: {
      type: Boolean,
      required: true,
    },
    categories: {
      ...
      if (param.ISBN === '' || param.title === '') {
        ...
        saveBookResource.store(param)
          .then(response => {
            ...
            app.$emit('update:is-create-book-sidebar-active', false)
          }).catch(error => {
            app.error = true
